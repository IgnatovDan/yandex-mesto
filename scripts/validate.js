...
но в коде нет такого переключения.
...
- Можно лучше: все значения по умолчанию не требуются, они дублируют явно передаваемые значения
  ...
  formList.forEach(formEl => {
    formEl.addEventListener('submit', (evt) => {
      ...
      evt.preventDefault();
- На странице у формы есть другие обработчики события submit у формы и их работу невозможно 
надежно отменить через новый обработчик события
Используйте другие способы для отмены их работы.
    });
